...
| built | `mt6768.dtb` 122,474 B (629 nodes, 2,538 properties, 413 `compatible`) as built in the sandbox config; the product config currently yields 89,053 B - see KNOWN-ISSUES "DTB size discrepancy", which is an open item, not a resolved one, 5 `dtbo` images, `dtbo.img` 371 235 B via the vendor's own `scripts/mkdtboimg.py` (page 4096, ids 0-4) |
