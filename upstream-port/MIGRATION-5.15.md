...
| series round-trip: `git am` of all 77 patches onto pristine `v5.15.220` reproduces the ported tree | **same git tree hash `3ef43034ffc629d0808703917f01aceb7cfe632e (tree; the *commit* is 2fab59f24)`** |
...
| series round-trip | `git am` of the 77 patches onto pristine `v5.15.220` yields tree **`3ef43034ffc629d0808703917f01aceb7cfe632e`** - identical to the tree that was compiled and linked. Assert the applied count as well as the hash: an empty glob leaves `git am` succeeding with nothing applied |
...
| built | `mt6768.dtb` 122,474 B (629 nodes, 2,538 properties, 413 `compatible`) - re-measured on the product tree by forced rebuild, matching the reference decompile exactly; every DTB size in these docs is now quoted with the tree/config/target that produced it (KNOWN-ISSUES 7.1) |
