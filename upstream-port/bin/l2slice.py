...
def main():
    ...
    ap.add_argument("--gate", default="MTK_DISP_BRINGUP",
                    help="Kconfig symbol the generated obj- lines are gated on ('' for bare obj-y). "
                         "Default MTK_DISP_BRINGUP because objects in obj-y are demanded by every "
                         "build, so a slice whose providers are not landed yet would break `make "
                         "vmlinux` for the whole tree.")
    ...
    def write_makefile():
        ...
        # The objects are demanded only when the bring-up switch is on. Bare `obj-y` was the shape
        # 0084/0085 shipped with, and it makes any tree that descends this directory unlinkable while
        # a provider is missing (507 undefined references at the 0085 tip - see
        # report/l2-wholetree-survey.md 6); the vendor gates the same directory the same way
        # (mt6768/Makefile: obj-$(CONFIG_MTK_FB) += dispsys/). `--gate ""` restores obj-y for probes
        # that deliberately want the objects built unconditionally.
        lhs = "obj-y" if not a.gate else "obj-$(CONFIG_%s)" % a.gate
        objlines = "\n".join("%s += %s" % (lhs, os.path.basename(o)[:-2] + ".o") for o in a.objs)
        body = ("#\n# MT6768 display core (ported subset of the vendor dispsys).\n#\n"
                ...
                "# 4.19.325 vendor source and is built by the slice gate.\n#" +
                ("" if not a.gate else "\n# Built only with CONFIG_%s=y (compile-verified bring-up objects, no\n"
                 "# functional claim); see the symbol's help text in\n"
        ...
        if rc == 0:
            ...
            if missing_o:
                ...
                if any(os.path.basename(m)[:-2] + ".c" in ln for ln in out2.split("\n")
                       if ln.startswith("obj-y") or ln.startswith("obj-$(CONFIG_")):
