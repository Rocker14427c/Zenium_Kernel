...
def main():
    ...
    disp = os.path.join(VENDOR_REL, "video/mt6768/dispsys")
    search_dirs = [d for d in STOCK_INCLUDE_DIRS] + [disp, os.path.join(VENDOR_REL, "include/mt-plat"),
    ...
    def write_makefile():
        ...
        inc = [d for d in STOCK_INCLUDE_DIRS if os.path.isdir(os.path.join(a.target, d))]
    ...
    inc = [d for d in STOCK_INCLUDE_DIRS if os.path.isdir(os.path.join(a.target, d))]
    skipped = [d for d in STOCK_INCLUDE_DIRS if d not in inc]
