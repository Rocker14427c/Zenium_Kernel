...
| **build-complete** | the ported tree compiles and links: `Image`, `vmlinux`, every DTB, and loadable modules | **DONE for the tree any user builds** - at the 0091 tip, config of record: `vmlinux` 168,340,520 B, `Image` 34,165,248 B, `Image.gz-dtb` 12,228,265 B, the device's own `mt6768.dtb` 122,474 B (sha `34a7e6b536a3…`), 0 `error:` lines and 0 undefined references. Two honest qualifiers: the display objects behind `CONFIG_MTK_DISP_BRINGUP` deliberately do **not** link (211 deferred reference lines / 62 distinct names, §Round 0083-0091), and the 529-DTB / 840-`.ko` figures are `build-37`'s - modules and the DTB sweep have not been re-measured since 0081 | `report/build-evidence.md`, `report/build.json` (gate `l2_disp_record_publish49`), `report/subsystem-audit.md` |
